...
{
    fn start_task(&self, backlog_size: usize, config: TaskConfig) -> (JoinHandle<()>, TenoriteClient<Request, Response, Error>) {
        let (sender, receiver) = mpsc::channel(backlog_size);
        let task = tokio::spawn(async move {
            <Task as TenoriteTask<Request, Response, Error, TaskConfig>>::task(receiver, config).await
        });
